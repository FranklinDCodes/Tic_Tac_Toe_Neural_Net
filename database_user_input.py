...
table = database.table["training_data_user_input_0"]
...
while True:
    ...
    # ask user if the network should be live trained on game
    do_train_raw = input("Train T3NN on this game?: ")
    do_train = do_train_raw == "YES"
    
    # create training examples from game and add them to database
    table.insertRows(g.get_training_examples(True))


    # if the user wants to train live
    if do_train:
        ...
        model.train()
        print(f"Network Training (LR={lr}) ... \n")

        # open training examples
        for input_seq, label in g.get_training_examples(False):

            # forward pass
            output = model(pyt.tensor(input_seq, dtype=pyt.float32))
